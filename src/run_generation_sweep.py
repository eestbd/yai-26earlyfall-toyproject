"""Generation-length sweep with the original 128-token prompt kept fixed."""
...
SETTINGS = PROJECT_ROOT / "docs" / "generation_sweep_config.json"
RESULT = PROJECT_ROOT / "results" / "generation_sweep_raw.json"


def run_one(model, inputs, config, record):
    device = torch.device(config["device"])
    prompt = record["prompt_tokens"]
    requested = record["requested_new_tokens"]
    ...
    try:
        ...
        require(list(outputs.shape) == [1, prompt + requested], "Generated-token count mismatch")
        ...
        if warmup:
            require(steps == requested, "Unexpected warm-up step count")
    ...
    print(f"P={prompt} G={requested} {'warm-up' if warmup else 'measured'} {record['repeat']} "
...
def main():
    ...
    require(settings["generation_lengths"] == [32,64,128,256] and settings["prompt_tokens"] == 128,
    ...
    require(settings["length_order"] == "ascending" and settings["use_original_fixed_input"] is True
    ...
    input_path = PROJECT_ROOT / settings["input_path"]
    require(hashlib.sha256(input_path.read_bytes()).hexdigest() == settings["input_file_sha256"], "Input file hash mismatch")
    fixed = load_fixed_input(input_path)
    ...
    report = {
        "status":"running", "scope":"generation_length_sweep_prompt_128",
        ...
        "source_sha256":{name:hashlib.sha256((PROJECT_ROOT/'src'/name).read_bytes()).hexdigest()
                         for name in ['run_generation_sweep.py','pilot_config.py','validate_fixed_input.py','calculate_throughput.py','summarize_pilot.py']},
    ...
    try:
        ...
        for requested in settings['generation_lengths']:
            configure_runtime(base)
            prompt = settings['prompt_tokens']
            config = copy.deepcopy(base)
            config['generation']['min_new_tokens'] = requested
            config['generation']['max_new_tokens'] = requested
            off = generation_config_for(config, False).to_dict()
            on = generation_config_for(config, True).to_dict()
            require([key for key in off if off[key] != on[key]] == ['use_cache'], "Unexpected ON/OFF configuration differences")
            require(prompt+requested <= model.config.max_position_embeddings, "Context exceeds model limit")
            condition = {'prompt_tokens':prompt, 'generation_tokens':requested, 'input_sha256':fixed['sha256'],
                         'generation_config_overrides':{'min_new_tokens':requested,'max_new_tokens':requested},
                         'warmup_runs':[], 'repeat_runs':[]}
            report['conditions'].append(condition)
            ids = torch.tensor([fixed['token_ids']],dtype=torch.long,device=base['device'])
            ...
            for warm,rep,pos,mode in schedule:
                ...
                record = {'execution_index':len(records),'prompt_tokens':prompt,'requested_new_tokens':requested,
                          'repeat':rep,'position_in_repeat':pos,'use_cache':mode,'warmup':warm,
                          'included_in_statistics':not warm,'status':'running'}
                records.append(record)
                run_one(model, inputs, config, record)
