"""Plot the validated prompt sweep with per-mode sample standard deviations."""
import hashlib
import json
import os
...
def main():
    ...
    lengths=[c['prompt_tokens'] for c in s['conditions']]
    ...
    for ax,(metric,title,ylabel,divisor) in zip(axes.flat,panels):
        ...
        ax.set(title=title,xlabel='Prompt length (tokens)',ylabel=ylabel,xticks=lengths)
    ...
    fig.suptitle('KV Cache ON/OFF: prompt length sweep',fontsize=20,fontweight='bold',y=.985)
    fig.text(.5,.935,'Qwen2.5-1.5B | RTX 3090 | BF16 | eager | generation 128 | batch 1',ha='center',fontsize=11)
    speeds='; '.join(f"{c['prompt_tokens']}: {c['speedup']:.2f}x" for c in s['conditions'])
    fig.text(.5,.075,'Means ± sample SD (n=5, ddof=1). Warm-up excluded. Inputs repeat the original 128 token IDs.',ha='center',fontsize=9)
    fig.text(.5,.05,'Latency speedup (OFF / ON) by prompt length: '+speeds,ha='center',fontsize=9)
    ...
    paths=[ROOT/'plots/prompt_sweep.png',ROOT/'plots/prompt_sweep.pdf']
    ...
    (ROOT/'results/prompt_sweep_plot_metadata.json').write_text(json.dumps(meta,indent=2)+'\n')
    print('Saved prompt sweep PNG/PDF.')
