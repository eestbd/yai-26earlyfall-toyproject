"""Validate and summarize the generation sweep without GPU execution."""
import csv
import hashlib
import json
import math
...
CSV=ROOT/'results'/'generation_sweep_raw.csv'
...
def main():
    ...
    require([c['generation_tokens'] for c in raw['conditions']]==[32,64,128,256],'Missing conditions')
    require(hashlib.sha256((ROOT/'docs/generation_sweep_config.json').read_bytes()).hexdigest()==raw['sweep_settings_sha256'],'Sweep config hash mismatch')
    for path,key in [(settings['base_config_path'],'base_config_sha256'),(settings['input_path'],'input_file_sha256')]:
    ...
    fixed=load_fixed_input(ROOT/settings['input_path'])
    require(raw['base_config']==json.loads((ROOT/settings['base_config_path']).read_text()),'Embedded base config mismatch')
    report={'status':'passed','recorded_at_utc':datetime.now(timezone.utc).isoformat(),
            ...
            'prompt_tokens':128,'generation_lengths':[32,64,128,256],'conditions':[]}
    rows=[]
    ...
    for c in raw['conditions']:
        requested=c['generation_tokens']
        require(c['prompt_tokens']==128 and c['input_sha256']==fixed['sha256'],'Input mismatch')
        require(c['generation_config_overrides']=={'min_new_tokens':requested,'max_new_tokens':requested},'Generation overrides mismatch')
        ...
        require(all(r['status']=='passed' and r['warmup'] and not r['included_in_statistics'] and r['diagnostic_hook_removed'] and r['finite_logits_steps']==requested and r['cache_path_verified'] for r in warm),'Warm-up validation failed')
        ...
        for r in warm+runs:
            require(r['status']=='passed' and r['actual_generated_tokens']==len(r['generated_token_ids'])==r['requested_new_tokens']==requested,'Token count mismatch')
        ...
        for r in runs:
            ...
            require(math.isclose(calculate_throughput(r['actual_generated_tokens'],r['total_latency_seconds']),r['throughput_tokens_per_second'],rel_tol=1e-12),'Throughput mismatch')
        ...
        result={'prompt_tokens':c['prompt_tokens'],'generation_tokens':requested,'input_sha256':c['input_sha256'],'statistics':stats,
    ...
    for c in report['conditions']:
        print(f"G={c['generation_tokens']}: OFF {c['statistics']['OFF']['total_latency_seconds']['mean']:.4f}s; ON {c['statistics']['ON']['total_latency_seconds']['mean']:.4f}s; speedup {c['speedup']:.3f}x; identical outputs={c['all_generated_token_ids_equal']}")
